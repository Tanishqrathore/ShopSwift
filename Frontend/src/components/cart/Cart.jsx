...
const Cart = () => {
  ...
  const { cart } = useSelector((state) => state.carts); // Redux se cart lo

  useEffect(() => {
    const syncCart = () => {
      dispatch(createUserCart(cart)); // 🛒 Backend me cart save karo
      dispatch(getUserCart()); // 🔄 Backend se latest cart fetch karo
    };

    syncCart();
  }, [dispatch]); // Component mount hone par chalega
